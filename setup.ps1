...
if ($Dsn -notmatch '^https://') {
    Die "DSN looks wrong. Expected: https://ingest.callistosignal.com/servers/<id>"
}

if (-not $Token) { $Token = $env:CALLISTO_TOKEN }
if (-not $Token) { $Token = Read-Host "Paste the server token (from the server page in Callisto)" }
if (-not $Token) {
    Die "Token is required (sent in the X-Callisto-Jupiter-Token header)."
}
